...
@click.group(
    help="Command line interface of MPRAlib, a library for MPRA data analysis."
)
def cli() -> None:
    pass
...
def reporter_sequence_design(input_file: str) -> None:
    ...
    """
    if not validate_tsv_with_schema(
        input_file, ValidationSchema.REPORTER_SEQUENCE_DESIGN
    ):
...
def reporter_barcode_to_element_mapping(input_file: str) -> None:
    ...
    """
    if not validate_tsv_with_schema(
        input_file, ValidationSchema.REPORTER_BARCODE_TO_ELEMENT_MAPPING
    ):
        raise click.ClickException("Validation failed. Please check the input file.")
...
def reporter_experiment_barcode(input_file: str) -> None:
    ...
    """
    if not validate_tsv_with_schema(
        input_file, ValidationSchema.REPORTER_EXPERIMENT_BARCODE
    ):
        raise click.ClickException("Validation failed. Please check the input file.")
...
def reporter_genomic_element(input_file: str) -> None:
    ...
    """
    if not validate_tsv_with_schema(
        input_file, ValidationSchema.REPORTER_GENOMIC_ELEMENT
    ):
        raise click.ClickException("Validation failed. Please check the input file.")
...
def reporter_genomic_variant(input_file: str) -> None:
    ...
    """
    if not validate_tsv_with_schema(
        input_file, ValidationSchema.REPORTER_GENOMIC_VARIANT
    ):
        raise click.ClickException("Validation failed. Please check the input file.")
...
def activities(
    input_file: str, bc_threshold: int, element_level: bool, output_file: str
) -> None:
    """
...
@functional.command(
    help="Compute pairwise correlations under a certain barcode threshold."
)
@click.option(
    "--input",
    "input_file",
...
def compute_correlation(
    input_file: str, bc_threshold: int, correlation_on: str, correlation_method: str
) -> None:
    """
    ...
    for method in correlation_method_list:
        for mode in correlation_on_mode:
            click.echo(
                f"{method} correlation on {mode}: {mpradata.correlation(method, mode).flatten()[[1, 2, 5]]}"
            )


@functional.command(
    help="Filter barcodes based on min/max counts, random or detect barcode outliers."
)
@click.option(
    "--input",
    "input_file",
...
@click.option(
    ...
    help="Barcode filtering method. Choices: "
    + ", ".join([f.name.lower() for f in BarcodeFilter]),
)
@click.option(
    "--method-values",
    "method_values",
        ...
        try:
            ...
            try:
                ...
                raise click.ClickException(
                    "Could not parse --method-values as dict or JSON."
                )
...
def get_variant_map(
    input_file: str, sequence_design_file: str, output_file: str
) -> None:
    """
        ...
        variant_map = pd.DataFrame(
            {
                ...
                "oligo": df_sequence_design.index.values.repeat(
                    df_sequence_design["SPDI"].apply(lambda x: len(x))
                ),
            }
        )
...
@combine.command(
    help="Return DNA and RNA counts for all oligos or only thosed tagges as elements/ref snvs."
)
@click.option(
    "--input",
    "input_file",
...
def get_counts(
    ...
    mpradata.add_sequence_design(
        read_sequence_design_file(sequence_design_file), sequence_design_file
    )
    ...
    if elements_only:
        ...
        element_mask = np.array(
            mpradata.data.var["allele"].apply(lambda x: "ref" in x).values, dtype=bool
        ) | (mpradata.data.var["category"] == "element")
        element_mask = ~np.repeat(
            np.array(element_mask)[np.newaxis, :], mpradata.n_obs, axis=0
        )

    export_counts_file(
        mpradata, output_file, normalized=normalized_counts, filter=element_mask
    )
...
def get_variant_counts(
    ...
    if use_oligos:
        ...
        ref_columns = [
            f"dna_count_{replicate}_REF" for replicate in mpradata.obs_names
        ] + [f"rna_count_{replicate}_REF" for replicate in mpradata.obs_names]
        alt_columns = [
            f"dna_count_{replicate}_ALT" for replicate in mpradata.obs_names
        ] + [f"rna_count_{replicate}_ALT" for replicate in mpradata.obs_names]
        nan_columns = df.loc[:, ref_columns].isna().all(axis=1) | df.loc[
            :, alt_columns
        ].isna().all(axis=1)
        df = df[~nan_columns]
...
@combine.command(
    help="Generate reporter element file from counts, sequence design and quantification statistics."
)
@click.option(
    "--input",
    "input_file",
...
def get_reporter_elements(
    ...
    indexes_in_order = [
        mpradata.oligos[mpradata.oligos == ID].index.tolist() for ID in df["ID"]
    ]
...
@combine.command(
    help="Generate reporter variant file from counts, sequence design and quantification statistics."
)
@click.option(
    "--input",
    "input_file",
...
def get_reporter_variants(
    ...
    df = pd.read_csv(
        statistics_file, sep="\t", header=0, index_col=0, na_values="NA"
    ).dropna()
...
@combine.command(
    help="Generate reporter genomic elements file from counts, sequence design and quantification statistics."
)
@click.option(
    "--input",
    "input_file",
...
@combine.command(
    help="Generate reporter genomic variants file from counts, sequence design and quantification statistics."
)
@click.option(
    "--input",
    "input_file",
...
def get_reporter_genomic_variants(
    ...
    for spdi, row in variant_map.iterrows():
        ...
        if spdi in df.index:
            ...
            df.loc[str(spdi), "strand"] = mpradata.data.var["strand"][
                mpradata.oligos.isin(row["REF"])
            ].values[0]
    ...
    with pysam.BGZFile(output_reporter_genomic_variants_file, "ab", None) as f:
        f.write(
            df.to_csv(sep="\t", index=False, header=False, float_format="%.4f").encode()
        )


def _get_chr(
    map: pd.DataFrame, variant_id: str, logger: logging.Logger
) -> Optional[str]:
    ...
    if variant_contig in map["refseq"].values:
        ...
        logger.warning(
            f"Contig {variant_contig} of SPDI {variant_id} not found in chromosome map. Returning None."
        )
        return None
...
@plot.command(
    help="Scatter plot to see correlations between replicates (activity, DNA or RNA)."
)
@click.option(
    "--input",
    "input_file",
...
@plot.command(
    help="Plotting the DNA vs RNA counts (log10, median on  multiple replicates)."
)
@click.option(
    "--input",
    "input_file",
...
def barcodes_per_oligo(
    input_file: str, replicates: Optional[list], output_file: str
) -> None:
    """
