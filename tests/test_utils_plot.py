...
def barcode_data():
    ...
    barcode_threshold = 1
    return copy.deepcopy(DummyMPRABarcodeData(
        replicates, oligos, barcodes,
...
def oligo_data():
    ...
    barcode_threshold = 1
    return copy.deepcopy(DummyMPRAOligoData(
        replicates, oligos, barcodes,
        dna_counts, rna_counts,
        barcode_threshold,
        barcode_counts
...
def test_correlation_masking_barcode(barcode_data):
    # Set barcode_counts below threshold for one oligo
    barcode_data.barcode_counts[0, 2] = 0
    g = correlation(barcode_data, Modality.DNA)
    # Should still return a PairGrid
    assert isinstance(g, sns.PairGrid)
